...
export function createWebTools(tracker?: ActionTracker) {
  return {
    ...
    web_crawl: tool({
      description: "Scrape a webpage URL into clean markdown text.",
      inputSchema: z.object({ url: z.string().url().describe("Target URL to scrape") }),
      execute: async ({ url }) => {
    ...
    fetch_url: tool({
      description: "Perform an HTTP GET request on a URL. Returns raw text/HTML.",
